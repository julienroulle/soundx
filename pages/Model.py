...
for idx in range(5):
    predictions_columns[0].metric(classes[scores_idx[idx]], f"{scores[scores_idx[idx]] * 100:.1f} %")

predictions_columns[1].subheader("General Prediction")
tflite_model = client.download_file(Bucket='soundx-models', Key=f'{model}/soundx_model_general.tflite', Filename='/tmp/soundx_model_general.tflite')
classes = json.loads(s3.Object("soundx-models", f"{model}/soundx_model_general.json").get()['Body'].read().decode('utf-8'))['classes']

scores, scores_idx = soundx_tflite_prediction(resampled_signal, tflite_model='/tmp/soundx_model_general.tflite')

for idx in range(5):
    predictions_columns[1].metric(classes[scores_idx[idx]], f"{scores[scores_idx[idx]] * 100:.1f} %")
...
tflite_model = client.download_file(Bucket='soundx-models', Key=f'{model}/soundx_model_{general_label}.tflite', Filename=f'/tmp/soundx_model_{general_label}.tflite')
classes = json.loads(s3.Object("soundx-models", f"{model}/soundx_model_{general_label}.json").get()['Body'].read().decode('utf-8'))['classes']

scores, scores_idx = soundx_tflite_prediction(resampled_signal, tflite_model=f'/tmp/soundx_model_{general_label}.tflite')

for idx in range(5):
    predictions_columns[2].metric(classes[scores_idx[idx]], f"{scores[scores_idx[idx]] * 100:.1f} %")

predictions_columns[3].subheader("Specific Prediction 2nd label")
tflite_model = client.download_file(Bucket='soundx-models', Key=f'{model}/soundx_model_{second_label}.tflite', Filename=f'/tmp/soundx_model_{second_label}.tflite')
classes = json.loads(s3.Object("soundx-models", f"{model}/soundx_model_{second_label}.json").get()['Body'].read().decode('utf-8'))['classes']

scores, scores_idx = soundx_tflite_prediction(resampled_signal, tflite_model=f'/tmp/soundx_model_{second_label}.tflite')

for idx in range(5):
    predictions_columns[3].metric(classes[scores_idx[idx]], f"{scores[scores_idx[idx]] * 100:.1f} %")
